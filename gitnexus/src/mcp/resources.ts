...
async function getContextResource(backend: LocalBackend, repoName?: string): Promise<string> {
  ...
  lines.push('');
  lines.push(
    're_index: Run `node gitnexus/dist/cli/index.js analyze` in terminal if data is stale',
  );
  lines.push('');
...
async function getSetupResource(backend: LocalBackend): Promise<string> {
  ...
  if (repos.length === 0) {
    return '# GitNexus\n\nNo repositories indexed. Run: `node gitnexus/dist/cli/index.js analyze` in a repository.';
  }
